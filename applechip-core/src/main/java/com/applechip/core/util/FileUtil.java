...
public class FileUtil {
	...
	private static OutputStream getOutputStream(HttpServletRequest request, HttpServletResponse response,
			String filename) throws IOException {
		response.setContentType(String.format("application/octet-stream; charset=%s", CoreConstant.CHARSET.toString()));
		String userAgent = StringUtils.defaultString(request.getHeader("User-Agent"));
		String header = "";
		if (StringUtils.contains(userAgent, "MSIE 5.5")) {
			header = String.format("filename=%s;", URLEncoder.encode(filename, CoreConstant.CHARSET.toString()));
			response.setHeader("Cache-Control", "no-cache");
			response.setHeader("Pragma", "no-cache");
		}
		else if (StringUtils.contains(userAgent, "MSIE")) {
			header = String.format("attachment; filename=%s;",
					URLEncoder.encode(filename, CoreConstant.CHARSET.toString()).replace("+", " "));
			response.setHeader("Cache-Control", null);
			response.setHeader("Pragma", null);
		}
		else {
			header = String.format("attachment; filename=\"%s\";", new String(filename.getBytes(CoreConstant.CHARSET),
					CoreConstant.CHARSET));
		}
