...
public class SchemaExportTask extends DefaultTask {
	...
	protected void start() {
		...
		try {
			...
			URLClassLoader hibernateToolsClassloader = new URLClassLoader(classpath.toArray(new URL[classpath.size()]),
					originalClassloader);
	...
	// public void test() {
	// Connection connection = DriverManager.getConnection(url, info)onnection("");
	// Dialect.getDialect(props) dialect = new MySQL5InnoDBDialect();
	// org.hibernate.cfg.Configuration cfg = new org.hibernate.cfg.Configuration();
	// cfg.addAnnotatedClass(CoreConfig.class);
	// cfg.buildMappings();
	// DatabaseMetadata metadata = new DatabaseMetadata(connection, dialect, cfg);
	...
	// formatted = FormatStyle.DDL.getFormatter().format(script.getScript());
	// formatted += ";";
